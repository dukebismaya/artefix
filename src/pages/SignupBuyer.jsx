...
export default function SignupBuyer() {
  ...
  function onSubmit(e) {
    ...
    setErr('')
    try {
      signup('buyer', { name, email, password })
      nav('/buyer')
    } catch (e) {
  ...
  return (
    <section className="mx-auto max-w-md px-4 sm:px-6 lg:px-8 pt-10 pb-16">
      <h2 className="text-2xl font-bold gradient-text">Buyer Sign Up</h2>
      <form onSubmit={onSubmit} className="card p-5 mt-4">
        ...
        <p className="mt-3 text-xs text-gray-400">Already have an account? <Link to="/login-buyer" className="text-teal-300">Log in</Link></p>
      </form>
    </section>
  )
}
