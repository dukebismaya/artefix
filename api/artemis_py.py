...
#   HF_CHAT_MODEL (default: mistralai/Mistral-7B-Instruct-v0.3)
...
def hf_text_generate(model: str, token: str, prompt: str) -> str:
    ...
    try:
        with urlrequest.urlopen(req, timeout=60) as resp:
            ...
            body = resp.read()
            if status == 404:
                raise FileNotFoundError('HF_NOT_FOUND')
            if status < 200 or status >= 300:
                raise RuntimeError(f'HF error {status}: {body[:200]!r}')
            try:
...
def hf_image_generate(model: str, token: str, prompt: str) -> str:
    ...
    with urlrequest.urlopen(req, timeout=120) as resp:
        ...
        if status == 404:
            raise FileNotFoundError('HF_NOT_FOUND')
        if status < 200 or status >= 300:
            raise RuntimeError(f'HF img error {status}: {data[:200]!r}')
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        hf_chat_model = sanitize_model_id(options.get('hfModel') or os.environ.get('HF_CHAT_MODEL') or 'mistralai/Mistral-7B-Instruct-v0.3')
        ...
        try:
            reply = hf_text_generate(hf_chat_model, hf_token, prompt)
            elapsed = int(time.time() * 1000) - t0
            self._set_headers(200, trace_id)
            self.wfile.write(json.dumps({'reply': reply, 'provider': 'huggingface', 'modelUsed': hf_chat_model, 'note': 'ok', 'elapsedMs': elapsed, 'traceId': trace_id}).encode('utf-8'))
            return
        except FileNotFoundError:
            # Try fallback model
            try:
                reply2 = hf_text_generate(hf_fallback_model, hf_token, prompt)
                elapsed = int(time.time() * 1000) - t0
                self._set_headers(200, trace_id)
                self.wfile.write(json.dumps({'reply': reply2, 'provider': 'huggingface', 'modelUsed': hf_fallback_model, 'note': 'fallback:HF_NOT_FOUND', 'elapsedMs': elapsed, 'traceId': trace_id}).encode('utf-8'))
                return
            except Exception as e2:
                pass
        except Exception as e:
            # Any other HF error -> local fallback
            pass

        reply = local_fallback(messages, context)
        elapsed = int(time.time() * 1000) - t0
        self._set_headers(200, trace_id)
        self.wfile.write(json.dumps({'reply': reply, 'provider': 'local-fallback', 'note': 'hf-error', 'elapsedMs': elapsed, 'traceId': trace_id}).encode('utf-8'))
        return
